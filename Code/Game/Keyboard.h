/**
 *  @file       Keyboard.h
 *  @brief      CKeyboard class interface
 *
 *  @author     Mark L. Short
 *  @date       May 7, 2017
 *
 *
 *  <b>Cite:</b> Following code was inspired by Microsoft DirectX Toolkit
 *
 */
   ...
   enum class Keys : int
    ...
    struct KeyboardState
    {
      ...
      //  { memset(this, 0, sizeof(*this) ); };
        ...
        bool  IsKeyStateSet(Keys key) const noexcept;
   };

typedef void (CALLBACK* KEYBOARD_HANDLER)(UINT nKey, BOOL bPressed);
...
private:
    ...
    {
    public:
        KeyboardState m_stReleased;
        KeyboardState m_stPressed;
        KeyboardState m_stLast;

        /// Default Constructor
        constexpr CStateTracker() noexcept
            : m_stReleased(),
              m_stPressed(),
              m_stLast()
        { };

        void  Update       (const KeyboardState& state) noexcept;

        void  InitKeyStates(void) noexcept;

        bool  IsKeyPressed (Keys key) const noexcept;
        bool  IsKeyReleased(Keys key) const noexcept;
        bool  IsKeyLast    (Keys key) const noexcept;
    };


   KeyboardState    m_State;
   CStateTracker    m_StateTracker;
   KEYBOARD_HANDLER m_pfHandler;
   
public:
    /// Default Constructor
    constexpr CKeyboard() noexcept
        : m_State(),
          m_StateTracker(),
          m_pfHandler(nullptr)
    { };
    ...
    constexpr const KeyboardState&  GetState (void) const noexcept;

    // initialize the keyboard state
    void          InitKeyStates    (void) noexcept;

    void          SetHandler       (KEYBOARD_HANDLER pfHandler) noexcept;

    inline void   UpdateStates     (void) noexcept
    { m_StateTracker.Update(m_State); };

    inline bool   IsKeyPressed     (Keys key) const noexcept
    { return m_StateTracker.IsKeyPressed(key); };

    inline bool   WasKeyJustPressed(Keys key) const noexcept
    { return m_StateTracker.IsKeyPressed(key) && !m_StateTracker.IsKeyLast(key); };
//======================================================================================
//
...
private:
    ...
    // prevent inadvertent copying or assignment

    /// Copy constructor
    CKeyboard(CKeyboard const&) = delete;
    /// Assignment operator
    CKeyboard& operator=(CKeyboard const&) = delete;
};


constexpr const KeyboardState& 
CKeyboard::GetState (void) const noexcept
{
    return m_State;
};
